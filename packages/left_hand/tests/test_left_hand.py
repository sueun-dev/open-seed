...
from openseed_left_hand.parser import (
    _try_extract_usage_from_stderr,
    parse_json_output,
    parse_output,
    parse_text_output,
)
from openseed_left_hand.roles import ROLES, Role, get_role
...
class TestRoles:
    ...
    def test_get_role_oracle(self):
        role = get_role("oracle")
        assert role.name == "oracle"
        assert role.model == "opus"
        assert role.thinking_budget == 20_000
        # Oracle cannot execute — only read tools
    ...
    def test_roles_registry_contains_expected_roles(self):
        expected = {"architect", "implementer", "reviewer", "debugger", "oracle"}
...
def agent(fake_config):
    ...
    with patch("openseed_left_hand.agent.require_claude_auth", return_value="/usr/local/bin/claude"):
...
class TestClaudeAgent:
    async def test_invoke_basic(self, agent):
        ...
        with patch("openseed_left_hand.agent.run_streaming", new_callable=AsyncMock, return_value=fake):
    ...
    async def test_invoke_continue_session(self, agent):
        ...
        with patch("openseed_left_hand.agent.run_streaming", new_callable=AsyncMock, return_value=fake1):
        ...
        with patch("openseed_left_hand.agent.run_streaming", new_callable=AsyncMock, return_value=fake2) as mock_run:
    ...
    async def test_invoke_model_shorthand_opus(self, agent):
        ...
        with patch("openseed_left_hand.agent.run_streaming", new_callable=AsyncMock, return_value=fake) as mock_run:
