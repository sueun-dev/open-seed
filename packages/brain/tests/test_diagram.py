...
class TestScanProjectFiles:
    ...
    def test_basic_scan(self, tmp_path: Any) -> None:
    ...
    def test_skips_lock_files(self, tmp_path: Any) -> None:
    ...
    def test_skips_hidden_and_node_modules(self, tmp_path: Any) -> None:
    ...
    def test_config_files_prioritized(self, tmp_path: Any) -> None:
    ...
    def test_entry_points_before_deep_files(self, tmp_path: Any) -> None:
        (tmp_path / "src").mkdir()
    ...
    def test_unsupported_extensions_skipped(self, tmp_path: Any) -> None:
        (tmp_path / "app.py").write_text("code")
    ...
    def test_reads_file_content(self, tmp_path: Any) -> None:
        (tmp_path / "hello.py").write_text("print('hello world')")
        result = scan_project_files(str(tmp_path))
        assert result[0]["content"] == "print('hello world')"

    def test_truncates_large_files(self, tmp_path: Any) -> None:
        large_content = "x = 1\n" * 5000  # ~30K chars
        (tmp_path / "big.py").write_text(large_content)
        result = scan_project_files(str(tmp_path))
        assert len(result[0]["content"]) <= 8500  # MAX_SINGLE_FILE + some margin

    def test_nested_directory_structure(self, tmp_path: Any) -> None:
...
class TestFixMermaidCycles:
    ...
    def test_detects_and_fixes_simple_cycle(self) -> None:
        code = """graph TD
    subgraph Extension
    ...
    def test_fixes_multiple_cycles(self) -> None:
        code = """graph TD
    subgraph API
        API[API Server]
    end
    ...
    def test_preserves_labels_with_brackets(self) -> None:
        code = """graph TD
...
class TestCreateMermaidUrl:
    ...
    def test_img_variant(self) -> None:
        url = create_mermaid_url("graph TD\n    A --> B", variant="img")
...
Some explanation."""
    ...
    def test_extracts_without_code_block(self) -> None:
        text = "graph TD\n    A --> B"
        result = _extract_mermaid(text)
        assert "graph TD" in result

    def test_returns_empty_for_no_diagram(self) -> None:
        result = _extract_mermaid("No diagram here, just text.")
        assert result == ""

    def test_handles_flowchart_prefix(self) -> None:
        text = "flowchart LR\n    A --> B"
        result = _extract_mermaid(text)
        assert "flowchart LR" in result

    def test_extracts_sequence_diagram(self) -> None:
        text = "```mermaid\nsequenceDiagram\n    A->>B: Hello\n```"
        result = _extract_mermaid(text)
...
class TestSmartTruncate:
    ...
    def test_preserves_head(self) -> None:
    ...
    def test_includes_signatures(self) -> None:
        lines = ["# header"] * 30
