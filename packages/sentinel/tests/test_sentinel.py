...
Coverage:
  ...
  - evaluate_loop: pass / retry / oracle / user_escalate / abort
"""
...
class TestEvaluateLoop:
    ...
    # ── ORACLE: stagnation threshold reached ─────────────────────────────────
    ...
    async def test_evaluate_loop_oracle_on_stagnation(self):
        ...
        # Oracle was consulted → retry with its advice
        assert decision.action == "retry"
        assert decision.oracle_advice is not None
        assert decision.oracle_advice.suggested_approach == "try something else"

    # ── USER_ESCALATE: stagnated + oracle already consulted ───────────────────
    ...
    # ── ABORT: oracle recommends abandonment ─────────────────────────────────

    async def test_evaluate_loop_abort_when_oracle_says_abandon(self):
        ...
        assert decision.action == "abort"
        assert "abandon" in decision.reason.lower() or "Oracle" in decision.reason
