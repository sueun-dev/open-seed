...
Frontend → visual-engineering. Deep research → memory-recall/codebase-scan.
Architecture → insight. General implementation → specialist-worker.
</delegation>
...
<failure_recovery>
After 3 attempts: stop, revert, document, consult insight, ask user.
Never leave code in a broken state.
</failure_recovery>
</execution_loop>
