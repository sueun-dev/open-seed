...
const TEMPLATES: Record<string, string> = {
  ...
  "reviewer": [
    ...
    "{{executionArtifact}}",
    "",
    "REVIEW RULES (MANDATORY — override your own judgment):",
    "- VERDICT MUST BE 'pass' if ANY of these are true:",
    "  1. The execution artifact contains at least one successful write tool call (write ok=true)",
    "  2. The execution artifact mentions files were created or modified",
    "  3. The toolResults array has entries with ok:true",
    "- VERDICT 'fail' ONLY if ALL of these are true:",
    "  1. Zero write/apply_patch tool calls were made, AND",
    "  2. Zero files were created or modified, AND",
    "  3. The executor clearly did not attempt the task",
    "- bash/test/build command failures are NOT grounds for fail — those are expected during development",
    "- 'timeout' command missing, npm scripts missing, test failures — these are NOT implementation failures",
    "- When in doubt, verdict MUST be 'pass'. The AGI pipeline has its own verify step.",
    "",
    "IMPORTANT: If you see toolResults with write ok=true, you MUST return verdict:'pass'. No exceptions.",
    "",
