...
const TEMPLATES: Record<string, string> = {
  ...
  "reviewer": [
    ...
    "",
    "REVIEW RULES:",
    "- If the execution artifact contains successful tool results (read ok, write ok), that IS evidence of implementation.",
    "- If files were written successfully, verdict should be 'pass' even if build/test commands failed (missing scripts is not a code error).",
    "- Only verdict 'fail' if: NO tool calls were made, OR write calls failed, OR the written code is clearly wrong.",
    "- 'npm test' or 'npm run build' failing because scripts are missing in package.json is NOT a failure of the implementation.",
    "",
